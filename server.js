...
app.get("/me", (req, res) => {
  ...
  res.json({ loggedIn: true, username: u.username, avatar: u.avatar, starredRooms: u.starredRooms, admin: !!u.admin });
});
...
app.post("/create-room", requireAuth, (req,res) => {
  ...
  for (const r of Object.values(store.rooms))
    if (r.name?.trim().toLowerCase()===lower) return res.json({ success:false, message:"Room name already in use" });
